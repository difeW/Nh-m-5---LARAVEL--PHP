...
class OrderController extends Controller
{
	...
	public function huy_don_hang(request $request){
		if($request->status == 'Đã xác nhận đơn hàng' || $request->status == 'Đơn hàng đang được giao')
		{
			DB::table('tbl_product')->where('order_id', $request->id)->update(['order_status'=>'Đơn hàng đã hủy']);
		}
		$kq= DB::table('tbl_order')->where('order_id', $request->id)->update(['order_status'=>'Đơn hàng đã hủy']);
		return $kq;
    }

	public function nhan_don_hang(request $request){
		$kq= DB::table('tbl_order')->where('order_id', $request->id)->update(['order_status'=>'Đơn hàng đã được nhận']);
		return $kq;
    }
	...
	public function show_detail_order($order_id, request $request)
	{
		...
		$order_details = DB::table('tbl_order_details')->join('tbl_product','tbl_order_details.product_id','tbl_product.product_id')->where('order_id',$order_id)->get();
        ...
        return view('pages.checkout.detail_order')->with(compact('order_details','customer','shipping','order','order_status'))->with('meta_desc',$meta_desc)->with('meta_keywords',$meta_keywords)->with('meta_title',$meta_title)->with('url_canonical',$url_canonical)->with('slider',$slider)->with('category',$cate_product)->with('brand',$brand_product);
	} 
}
